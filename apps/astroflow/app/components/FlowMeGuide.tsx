...
export default function FlowMeGuide({ state, me }: { state: GuideState; me: string }) {
  ...
  if (!state.hasProfile) steps.push({ label: t('✦ Add your chart'), href: '/profile/new', primary: true });
  if (state.souls > 0) steps.push({ label: state.souls > 1 ? t('Send {n} activation links', { n: state.souls }) : t('Send {n} activation link', { n: state.souls }), onClick: () => document.getElementById('souls')?.scrollIntoView({ behavior: 'smooth' }), primary: state.hasProfile });
  if (state.friends === 0) steps.push({ label: t('+ Invite your first friend'), onClick: () => setOpen(true), primary: state.hasProfile && state.souls === 0 });
  steps.push({ label: t('Chart someone instantly'), href: '/instant' });
  steps.push({ label: t('Weave a constellation'), href: '/' });
  if (state.constellations > 0) steps.push({ label: t('Open a saved constellation'), href: '/' });
  steps.push({ label: t('Study the cosmos ✦'), href: '/cosmos' });

  const greeting = !state.hasProfile
    ? t('Welcome to your sky. Start by adding your chart — your star — and everything else opens from there.')
    : state.souls > 0
      ? state.souls > 1
        ? t('You’ve charted {n} souls waiting to activate. Send their links and watch them light up your constellation.', { n: state.souls })
        : t('You’ve charted {n} soul waiting to activate. Send their links and watch them light up your constellation.', { n: state.souls })
      : state.friends === 0
        ? t('Your chart is set. Now invite a friend — share your bond link and your circle begins to glow.')
        : state.friends === 1
          ? t('Your circle holds {n} soul. Weave them into a constellation, or ask me anything below.', { n: state.friends })
          : t('Your circle holds {n} souls. Weave them into a constellation, or ask me anything below.', { n: state.friends });
        ...
        <div className="relative">
          ...
          {open && (
            <div className="mt-4 rounded-xl border border-[#9a8fe0]/30 bg-[#11131f] p-4" style={{ animation: 'af-rise 0.35s ease-out' }}>
              <p className="text-sm text-[#cfc8e8] mb-3">{t('Two easy ways to grow your circle:')}</p>
              <div className="flex flex-wrap items-center gap-3 mb-3">
                <BondInvite />
                <span className="text-[11px] text-[#5b5e72]">{t('share your link — they bond the moment they accept')}</span>
              </div>
              ...
              <p className="text-[10px] text-[#5b5e72] mt-1.5">{t('Opens their star — request to see their chart if they keep it close.')}</p>
            </div>
          )}
            ...
            <div className="mt-4 space-y-2">
              {thread.map((m, i) => (
                <p key={i} className={`text-sm leading-relaxed ${m.role === 'user' ? 'text-[#9698a8]' : 'font-serif text-[#ece9e0]'}`}>
                  {m.role === 'user' ? <span className="text-[#5b5e72]">{t('you · ')}</span> : <span className="text-[#b6abec]">FlowMe · </span>}
                  {m.text}
                </p>
              ))}
            </div>
          ...
          <div className="mt-4 flex gap-2">
            <input
              ...
              placeholder={t('Ask FlowMe — how do I…?')}
