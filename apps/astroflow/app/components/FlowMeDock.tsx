            ...
            <div ref={scroll} className="flex-1 overflow-auto px-4 py-3 space-y-2.5 min-h-[120px]">
              {thread.length === 0 && (
                <>
                  <p className="font-serif text-[15px] text-[#ece9e0] leading-relaxed">
                    {t('Stuck or curious? Ask me anything — how to read your Atlas, weave a constellation, or set up your profile.')}
                  </p>
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {QUICK.map((q) => (
                      <button key={q} onClick={() => ask(t(q))} className="text-[11px] px-2.5 py-1 rounded-full border border-[#2c3350] text-[#cfc8e8] active:bg-[#1a1f33] transition">{t(q)}</button>
                    ))}
                  </div>
                </>
              )}
              {thread.map((m, i) => (
                <p key={i} className={`text-sm leading-relaxed ${m.role === 'user' ? 'text-[#9698a8]' : 'font-serif text-[#ece9e0]'}`}>
                  {m.role === 'user' ? <span className="text-[#5b5e72]">{t('you · ')}</span> : <span className="text-[#b6abec]">{t('FlowMe · ')}</span>}{m.text}
                </p>
              ))}
              {busy && <p className="text-sm text-[#b6abec] font-serif">{t('FlowMe · ')}<span className="opacity-60">{t('listening to the stars…')}</span></p>}
            </div>
