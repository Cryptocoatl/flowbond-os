  ...
  return (
    <div>
        ...
        <div className="space-y-2 mb-4">
          {guests.map((g) => (
            <div key={g.id} className="flex items-center gap-2 text-sm">
              ...
              {g.claimed ? (
                <span className="text-[10px] uppercase tracking-wide text-[#7bd0c6]">claimed ✓</span>
              ) : (
                <>
                  {g.claim_code && (
                    <button
                    ...
                    >
                      {copied === g.claim_code ? 'Copied ✓' : 'Copy their invite'}
                    </button>
                  )}
      ...
      <p className="text-xs text-[#9698a8] mb-3">
        Add someone with their birth moment — their chart joins this weave instantly. They won&apos;t be
        linked to the flow until they claim the personalized invite you send them.
      </p>
      <div className="space-y-2">
        <input className={input} placeholder="Their name" value={form.name} onChange={(e) => set('name', e.target.value)} />
        ...
        >
          {busy ? 'Computing their chart…' : 'Weave them in'}
        </button>
