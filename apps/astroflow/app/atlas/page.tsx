...
export default async function AtlasPage() {
  ...
  if (!me)
    return (
      <Shell>
        <h1 className="text-2xl font-serif mb-2">{t('Your Atlas')}</h1>
        <p className="text-[#9698a8] mb-5">{t('Sign in to see where your stars fall across the earth.')}</p>
  ...
  if (!prof?.chart)
    return (
      <Shell>
        <h1 className="text-2xl font-serif mb-2">{t('Your Atlas')}</h1>
        <p className="text-[#9698a8] mb-5">{t('Create your chart first, then your power lines will appear on the globe.')}</p>
  ...
  return (
    <Shell wide>
      <Backdrop />
      <p className="text-[11px] uppercase tracking-[0.3em] text-[#b6abec] mb-1">{t('Astrocartography')}</p>
      <h1
      ...
      <p className="text-sm text-[#9698a8] mt-2 mb-5 max-w-2xl">
        {t('Every planet in your chart traces four lines across the earth. Stand on one and that planet’s energy turns up —')} <b className="text-[#cfc8e8]">MC</b> {t('for calling,')} <b className="text-[#cfc8e8]">IC</b> {t('for home,')} <b className="text-[#cfc8e8]">AC</b> {t('for vitality,')} <b className="text-[#cfc8e8]">DC</b> {t('for connection. Tap any line to read it and see the cities it runs through, filter by planet, angle or a theme ({themes}), and zoom into any region of the world.', { themes: `${t('Love')}, ${t('Career')}, ${t('Home')}…` })}
      </p>
      ...
      <p className="text-xs text-[#5b5e72] mt-4">
        {t('Want to see where your lines cross a friend’s? Open a collective’s Atlas from its')}{' '}
        <Link href="/dashboard" className="text-[#b6abec] underline">{t('flow map')}</Link> {t('— that’s where the power spots light up.')}
      </p>
    </Shell>
  );
}
