...
export default function NewProfile() {
  ...
  return (
    <div className="max-w-lg mx-auto p-6 text-[#ece9e0]">
      <h1 className="text-3xl font-serif mb-1">{t('Add your chart')}</h1>
      <p className="text-[#9698a8] text-sm mb-6">{t('Computed to the degree from your birth moment, then woven into the constellation.')}</p>

      <label className="text-xs uppercase tracking-wider text-[#5b5e72]">{t('Display name')}</label>
      <input className={input} value={form.displayName} onChange={(e) => set('displayName', e.target.value)} />

      <label className="text-xs uppercase tracking-wider text-[#5b5e72] mt-4 block">{t('Handle')} <span className="lowercase tracking-normal text-[#5b5e72]">{t('(optional — your @ on AstralFlow)')}</span></label>
      <div className="flex items-center gap-1">
        <span className="text-[#9698a8]">@</span>
        <input className={input} placeholder={t('auto from your FlowBond ID if blank')} value={form.handle}
          onChange={(e) => set('handle', e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} />
      </div>

      <div className="grid grid-cols-2 gap-3 mt-4">
        <div>
          <label className="text-xs uppercase tracking-wider text-[#5b5e72]">{t('Birth date')}</label>
          <input type="date" className={input} value={form.date} onChange={(e) => set('date', e.target.value)} />
        </div>
        <div>
          <label className="text-xs uppercase tracking-wider text-[#5b5e72]">{t('Birth time')}</label>
      ...
      <label className="flex items-center gap-2 mt-2 text-xs text-[#9698a8]">
        ...
        {t('I don’t know my birth time (skips houses & rising)')}
      </label>

      <label className="text-xs uppercase tracking-wider text-[#5b5e72] mt-4 block">{t('Birth place')}</label>
      <PlaceAutocomplete
        value={form.place}
        onSelect={(r) =>
      ...
      {form.lat && form.lng && (
        <p className="text-[11px] text-[#7fd1a8] mt-1">
          ✓ {form.tz || t('tz unresolved')} · {parseFloat(form.lat).toFixed(4)},{' '}
          {parseFloat(form.lng).toFixed(4)} (E+)
        </p>
      )}
      {/* Manual override / fallback when no geocoder token is configured */}
      <details className="mt-2">
        <summary className="text-[10px] text-[#5b5e72] cursor-pointer">{t('Enter coordinates manually')}</summary>
        <div className="grid grid-cols-3 gap-3 mt-2">
          <input className={input} placeholder={t('IANA tz')} value={form.tz} onChange={(e) => set('tz', e.target.value)} />
          <input className={input} placeholder={t('lat')} value={form.lat} onChange={(e) => set('lat', e.target.value)} />
          <input className={input} placeholder={t('lng (E+)')} value={form.lng} onChange={(e) => set('lng', e.target.value)} />
        </div>
      </details>

      <label className="text-xs uppercase tracking-wider text-[#5b5e72] mt-6 block mb-2">{t('Who can see this chart?')}</label>
      <div className="space-y-2">
        {TIERS.map((tier) => (
          <button key={tier.v} onClick={() => setVisibility(tier.v)}
            className={`w-full text-left p-3 rounded-lg border transition ${visibility === tier.v ? 'border-[#e3c07a] bg-[#e3c07a]/10' : 'border-[#242a3b] bg-[#11131f]'}`}>
            <div className="text-sm font-medium">{t(tier.label)}</div>
            <div className="text-xs text-[#9698a8]">{t(tier.help)}</div>
          </button>
        ))}
      </div>
      ...
      <button onClick={submit} disabled={busy}
        ...
        {busy ? t('Computing…') : t('Compute & save my chart')}
      </button>
    </div>
  );
}
