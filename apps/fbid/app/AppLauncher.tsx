  ...
  return (
    <div className="space-y-3">
      ...
      <div className="hidden sm:flex justify-center py-4">
        <div className="relative w-[420px] h-[420px]">
          <div
            ...
            style={{ background: avatarGradient(identitySeed), opacity: 0.06, filter: 'blur(40px)' }}
            aria-hidden
          />
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    transition={reduceMotion ? undefined : { duration: RING_DURATION, repeat: Infinity, ease: 'linear' }}
                  >
                    <WorldNode world={w} index={i} onOpen={() => setActive(w)} reduceMotion={!!reduceMotion} />
                  </motion.div>
                </div>
              )
            })}
      ...
      <div className="grid grid-cols-2 gap-3 sm:hidden">
        {WORLDS.map((w, i) => (
          <WorldNode key={w.slug} world={w} index={i} onOpen={() => setActive(w)} reduceMotion={!!reduceMotion} asCard />
        ))}
      </div>

      <AnimatePresence>
        {active && <Portal world={active} onClose={() => setActive(null)} />}
      </AnimatePresence>
    </div>
  )
}
    ...
    return (
      <button
        ...
        className={`group flex items-center gap-3 text-left bg-white/[0.03] border border-white/10 rounded-2xl p-3 transition ${
          disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/[0.06] hover:border-violet-500/40'
        }`}
      >
        {content}
      </button>
  ...
  return (
    <motion.button
      ...
      className={`w-16 h-16 rounded-2xl bg-white/[0.04] border border-white/10 flex items-center justify-center backdrop-blur-sm transition-colors ${
        disabled ? 'opacity-40 cursor-not-allowed' : 'hover:bg-white/[0.09] hover:border-violet-500/50 shadow-lg'
      }`}
      title={disabled ? `${world.label} — coming back soon` : world.label}
    >
      {content}
    </motion.button>
  )
...
function Portal({ world, onClose }: { world: World; onClose: () => void }) {
  ...
  return (
    ...
    >
      <motion.div
        className="relative w-full max-w-sm bg-[#0c0c14] border border-white/10 rounded-3xl p-8 flex flex-col items-center gap-4 text-center shadow-2xl"
      ...
      >
        <div
          ...
          aria-hidden
        />
        <div className="w-28 h-28 rounded-3xl bg-white/[0.04] border border-white/10 flex items-center justify-center shadow-[0_0_60px_rgba(139,92,246,0.25)]">
          <img src={world.logo} alt="" className="w-20 h-20 object-contain" />
        </div>
        <div className="space-y-1">
          <h2 className="text-white text-lg font-bold">{world.label}</h2>
