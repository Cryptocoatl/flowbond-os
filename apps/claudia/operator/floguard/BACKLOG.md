...
| FG-062 | **HIGH:** Holy Honey data-room access code is a shared 4-digit value hardcoded as a fallback in `src/app/holy-honey/server.ts:23` **and** written literally into the committed `README.md`, together with the viewer allow-list. Verified live: the published code unlocks the partnership data room (`{"ok":true,"viewer":"Steph"}`). Repo is private, so not world-readable — but every repo collaborator holds the keys to the partnership documents, and 4 digits @ 5/10min is brute-forceable. | steph-manual | blocked (steph) | Drafted: set a strong `HH_ACCESS_CODE` in Vercel env, remove the `?? "1440"` fallback so it **fails closed**, scrub the README. Gated on Steph because rotating locks out Earl/Mariano/Sydney until they are told the new code (owner-never-locked-out rule). |
