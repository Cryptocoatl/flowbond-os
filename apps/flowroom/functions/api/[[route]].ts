...
app.post('/open', async (c) => {
  ...
  if (who.toLowerCase() === 'vandana' && (key ?? '').trim() === '1440') {
    return c.json({ ok: true, name: 'Vandana', role: 'owner' as Role });
  }
...
app.post('/key', async (c) => {
  ...
  if ((name ?? '').trim().toLowerCase() !== 'vandana' || (key ?? '').trim() !== '1440') {
    return c.json({ ok: false }, 401);
  }
  if (!role || !['editor', 'commenter', 'viewer'].includes(role)) return c.json({ ok: false }, 400);

  return c.json({ ok: true, token: await mint(c.env, 'Vandana', role) });
});
...
app.post('/notes', async (c) => {
  ...
  if (b.token) {
    ...
    role = p.r;
    invitedBy = p.f;
  } else if (author.toLowerCase() === 'vandana' && (b.key ?? '').trim() === '1440') {
    role = 'owner';
    author = 'Vandana';
  }
