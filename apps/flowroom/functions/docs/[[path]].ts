...
const OWNER_NAME = 'vandana';
const OWNER_KEY = '1440';
...
async function auth(request: Request, env: Env): Promise<Response> {
  ...
  /* Someone arriving on a key Vandana handed them. */
  if (b.token) {
  ...
  if (who.toLowerCase() === OWNER_NAME && (b.key ?? '').trim() === OWNER_KEY) {
    const pass: Pass = { f: 'Vandana', r: 'owner', n: 'Vandana' };
    const token = await mint(env.ROOM_SECRET, pass);
    return json({ ok: true, name: 'Vandana', role: 'owner', docs: null, token }, 200, {
      'set-cookie': setCookie(token),
    });
  }
