...
4. **Update the ClaudIA dashboard snapshot.** Regenerate
   `apps/claudia/lib/claudia/floguard-status.json` from the backlog so the
   "Seguridad del Imperio · FloGuard" panel stays current: refresh `lastAudit`,
   `advisors`, `critical`, `counts` (total/open/inProgress/blocked/done),
   `rotationsQueued`, `posture`, `headline`, and the `active` list (the
   in-progress + open items). Counts/metadata only — NEVER a secret value. The
   panel is client-only and reads this file at build, so the dashboard reflects
   it on the next CLI deploy of claudia. (claudia lives on branch `claudia-m1`;
   if a round runs from `main`/cloud, open a PR to `claudia-m1` with the updated
   json rather than pushing — claudia deploys via CLI only.)
5. If anything is CRITICAL, surface it to Steph immediately; don't wait.
